...
$(function () {
    ...
    */
    ...
    function setValue(data) {
        var setValue = $(".tx-inverse p");
        var i = 0;
        var sensor = data.sensor;
        $.each(setValue, function (index, item) {
            if (i == 0) {
                $(item).text(`${sensor[i].value} °C`);
            } else {
                $(item).text(`${sensor[i].value} %`);
            }
            i = i + 1;
        });
        process_time = sensor[0].process_time;
    };

    /**
     * Lấy dữ liệu cảm biến nhiệt độ, độ ẩm, ánh sáng, không khí
     */
    ...
    /*
        Sự kiện thay đổi số phút
    */
    function convertDateToTimestamp(date) {
        return Math.round(date.getTime() / 1000);
    }

    function getMinuteLastSensor(time) {
